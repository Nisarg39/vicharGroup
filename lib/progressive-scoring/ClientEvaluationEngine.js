...
export class ClientEvaluationEngine {
    ...
    async evaluateAnswerUpdate(questionId, answer) {
        ...
        try {
            ...
            this.updateMetrics('single_evaluation', evaluationTime);
    ...
    async updateProgressiveResults(questionId, evaluationResult) {
        if (!this.progressiveResults) {
            this.progressiveResults = {
                totalScore: 0,
                totalMarks: this.examData.exam.totalMarks || 0,
                percentage: 0,
                attempted: 0,
                correct: 0,
                incorrect: 0,
        ...
        // Update totals
        this.progressiveResults.totalScore = Object.values(this.currentAnswers).reduce((total, _, index) => {
            // This is a simplified calculation - the actual implementation would use proper scoring
            return total + (evaluationResult.isCorrect ? evaluationResult.marks : 0);
        }, 0);
        
        this.progressiveResults.percentage = this.progressiveResults.totalMarks > 0 
            ? (this.progressiveResults.totalScore / this.progressiveResults.totalMarks) * 100 
            : 0;
