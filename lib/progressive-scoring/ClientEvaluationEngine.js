...
export class ClientEvaluationEngine {
    ...
    async initialize(examData) {
        ...
        try {
            ...
            // DEBUG: Log initial progressive results state
            debugLogger.logExamEvent('CLIENT_ENGINE_INITIALIZED', {
                progressiveResults: JSON.parse(JSON.stringify(this.progressiveResults)),
                examId: this.examData.exam?.id,
                questionCount: this.examData.questions.length,
                totalMarks: this.progressiveResults.totalMarks,
                timestamp: Date.now()
            });
    ...
    async updateProgressiveResults(questionId, evaluationResult) {
        // DEBUG: Log entry with current state
        debugLogger.logExamEvent('PROGRESSIVE_UPDATE_START', {
            questionId,
            evaluationResult,
            currentProgressiveResults: this.progressiveResults ? JSON.parse(JSON.stringify(this.progressiveResults)) : null,
            currentAnswersCount: Object.keys(this.currentAnswers).length,
            timestamp: Date.now()
        });

        if (!this.progressiveResults) {
            debugLogger.error('CRITICAL: progressiveResults is null during update - reinitializing', {
                questionId,
                evaluationResult,
                examDataExists: !!this.examData,
                timestamp: Date.now()
            });
        ...
        // DEBUG: Log the complete updated state
        debugLogger.logExamEvent('PROGRESSIVE_UPDATE_COMPLETE', {
            questionId,
            updatedProgressiveResults: JSON.parse(JSON.stringify(this.progressiveResults)),
            calculationDetails: {
                totalScore,
                correct,
                incorrect,
                attempted: this.progressiveResults.attempted,
                totalMarks: this.progressiveResults.totalMarks,
                percentage: this.progressiveResults.percentage
            },
            questionResultsCount: Object.keys(this.progressiveResults.questionResults).length,
            currentAnswersSnapshot: JSON.parse(JSON.stringify(this.currentAnswers)),
            timestamp: Date.now()
        });

        // DEBUG: Check for zero value anomalies
        if (this.progressiveResults.totalScore === 0 && this.progressiveResults.attempted > 0) {
            debugLogger.warn('ZERO SCORE DETECTED with answered questions', {
                questionId,
                totalScore: this.progressiveResults.totalScore,
                attempted: this.progressiveResults.attempted,
                correct: this.progressiveResults.correct,
                questionResults: JSON.parse(JSON.stringify(this.progressiveResults.questionResults)),
                currentAnswers: JSON.parse(JSON.stringify(this.currentAnswers)),
                timestamp: Date.now()
            });
        }

        if (this.progressiveResults.totalMarks === 0) {
            debugLogger.error('ZERO TOTAL MARKS DETECTED', {
                examTotalMarks: this.examData?.exam?.totalMarks,
