...
const CartCard = () => {
    ...
    return (
        <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-gray-200 h-full">
                ...
                {student.cart.length > 0 ? (
                    <>
                        {student.cart.map((item, index) => (
                            <li key={index} className="p-4 bg-blue-50 rounded-xl hover:bg-blue-100 transition-all duration-300 font-medium border border-blue-100">
                        ...
                        <div className="mt-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-bold">{student.cart.length}</span>
                            </div>
                        </div>
                    </>
                ) : (
                    <li className="p-6 bg-blue-50 rounded-xl text-center">
...
const ProductsCard = () => {
    ...
    return (
        <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-gray-200 h-full">
            <h2 className="text-xl mb-4 text-gray-800 border-b pb-3 font-semibold flex items-center gap-2">
                <svg className="w-6 h-6 text-[#1d77bc]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <ul className="space-y-3">
                {student.purchases.length > 0 ? (
                        ...
                        <li key={index} className="p-4 bg-blue-50 rounded-xl hover:bg-blue-100 transition-all duration-300 font-medium border border-blue-100">
                            <div className="flex justify-between items-center">
                                <div className="flex gap-4 items-center">
                                    ...
                                    />
                                    <div className="space-y-1">
                                        <p className="text-gray-800 font-semibold">{purchase.product.name}</p>
                                        <span className="text-sm text-gray-500">Purchase Date: {new Date().toLocaleDateString()}</span>
                                    </div>
                                </div>
                                {purchase.product.type === "test-series" && (
                                    <button 
                                        className="bg-gradient-to-r from-[#1d77bc] to-[#2d8bd4] hover:from-[#1a6cad] hover:to-[#2884cc] text-white font-bold py-2.5 px-5 rounded-xl transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#1d77bc] focus:ring-opacity-50 group flex items-center gap-2"
                    ...
                    <li className="p-6 bg-blue-50 rounded-xl text-center">
                        ...
                        <p className="text-sm text-gray-500 mt-1">Your purchased courses will appear here</p>
                    </li>
                )}
            </ul>
        </div>
