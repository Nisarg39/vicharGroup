...
function DirectorMessage() {
    ...
    return (
        <section className="bg-gradient-to-r from-primary-100 to-primary-200 text-primary-900 py-8 sm:py-12 md:py-16 lg:py-24 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="bg-white/90 backdrop-blur-md p-4 sm:p-6 md:p-8 rounded-3xl shadow-2xl border-2 border-gray-100" style={{ backgroundImage: 'url(https://st3.depositphotos.com/1891407/12557/v/950/depositphotos_125579868-stock-illustration-vector-sketch-back-to-school.jpg)', backgroundSize: 'contain', backgroundPosition: 'center', }}>
                        <div className="bg-white/95 p-4 sm:p-6 md:p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,_0,_0,_0.7)] hover:shadow-[0_20px_60px_rgba(0,_0,_0,_0.9)] transition-all duration-300 border-2 border-gray-100">
                            <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6 md:gap-8">
                                ...
                                <div className="w-full md:w-2/2">
                                    <div 
                                        ref={scrollRef} 
                                        className="h-54 sm:h-40 md:h-48 overflow-y-auto mb-4 pr-4 flex items-center justify-center pt-2 md:pt-4 scrollbar-thin scrollbar-thumb-primary-500 scrollbar-track-primary-100" 
                                        style={{ scrollbarWidth: 'thin', scrollbarColor: '#106fb8 #f1f1f1' }}
                                        onTouchStart={handleTouchStart}
                                        onTouchEnd={handleTouchEnd}
                                    >
                                        {activeIndex === 0 && (
                                            <p className="text-sm sm:text-sm md:text-base lg:text-lg leading-relaxed text-left animate-fade-in">
                                                To all aspiring candidates with big dreams, remember that success is a journey that begins with the right start. If your goal is to crack competitive exams like <span className='text-[#e96030] font-semibold hover:text-primary-700 transition-colors duration-300'>NEET, IIT-JEE, MHT-CET, FOUNDATION</span> or to gain a solid foundation in the Stock Market, Vichar Group is here to guide you every step of the way.
                                            </p>
                                        )}
                                        {activeIndex === 1 && (
                                            <p className="text-sm sm:text-sm md:text-base lg:text-lg leading-relaxed text-left animate-fade-in">
                                    ...
                                    <div className="flex justify-between items-center mt-4 sm:mt-6">
                                        <button onClick={handlePrev} className="bg-[#106fb8] text-white px-3 sm:px-4 md:px-6 py-2 sm:py-3 rounded-lg hover:bg-[#f8f8f8] hover:text-[#106fb8] transition-all duration-300 transform hover:scale-110 shadow-lg hover:shadow-xl text-sm sm:text-base">←</button>
                                        <div className="flex space-x-1 sm:space-x-2">
                                            {[...Array(totalPoints)].map((_, index) => (
                                                <div
                                                    key={index}
                                                    className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-all duration-300 transform hover:scale-125 cursor-pointer ${index === activeIndex ? 'bg-[#106fb8] scale-110' : 'bg-gray-300'}`}
                                                    onClick={() => setActiveIndex(index)}
                                                ></div>
                                            ))}
                                        </div>
                                        <button onClick={handleNext} className="bg-[#106fb8] text-white px-3 sm:px-4 md:px-6 py-2 sm:py-3 rounded-lg hover:bg-[#f8f8f8] hover:text-[#106fb8] transition-all duration-300 transform hover:scale-110 shadow-lg hover:shadow-xl text-sm sm:text-base">→</button>
                                    </div>
                                </div>
                            </div>
                        </div>
