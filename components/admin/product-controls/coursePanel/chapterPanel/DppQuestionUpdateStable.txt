...
export default function DppQuestionUpdate({dpp, question, updateQuestion}) {
    ...
    // Helper function to process formatting (LaTeX, bold, italic) for a single line
    const processTextFormatting = (text) => {
        ...
        // If it's just a string (no LaTeX found), process bold and italic formatting
        if (typeof latexProcessed === 'string') {
            // Process bold formatting
            const boldPattern = /\*\*(.*?)\*\*/g;
            let parts = latexProcessed.split(boldPattern);
            ...
            return (
                <>
                    {parts.map((part, index) => {
                        // If this is a bold part (every odd index in the array)
                        if (index % 2 === 1) {
                            return <strong key={index}>{part}</strong>;
                        }
                        
                        // For non-bold parts, check for italic formatting
                                ...
                                <span key={index}>
                                    {italicParts.map((italicPart, italicIndex) => {
                                        return italicIndex % 2 === 0 ? 
                                            <span key={`${index}-${italicIndex}`}>{italicPart}</span> : 
                                            <em key={`${index}-${italicIndex}`}>{italicPart}</em>;
                                    })}
                                </span>
                            );
                        }
                        
                        // If no italic formatting, return the part as is
        ...
        // If we have an array of elements from LaTeX processing, we need to process each text element for formatting
        return (
            <>
                {latexProcessed.map((part, index) => {
                    ...
                    const boldPattern = /\*\*(.*?)\*\*/g;
                    let boldParts = part.split(boldPattern);
                    
                    if (boldParts.length > 1) {
                        return (
                            <span key={index}>
                                {boldParts.map((boldPart, boldIndex) => {
                                    // If this is a bold part
                                    if (boldIndex % 2 === 1) {
                                    ...
                                    if (italicParts.length > 1) {
                                        return (
                                            <span key={`${index}-${boldIndex}`}>
                                                {italicParts.map((italicPart, italicIndex) => {
                                                    return italicIndex % 2 === 0 ? 
                                                        <span key={`${index}-${boldIndex}-${italicIndex}`}>{italicPart}</span> : 
                                                        <em key={`${index}-${boldIndex}-${italicIndex}`}>{italicPart}</em>;
                                                })}
                                            </span>
                                        );
                                    }
                    ...
                    // If no bold formatting, check for italic formatting
                    const italicPattern = /\*(.*?)\*/g;
                    const italicParts = part.split(italicPattern);
